...
function buzz(p=10){ if(navigator.vibrate) navigator.vibrate(p); }
function showStep(i){
  current=i;
...
document.querySelectorAll(".next").forEach(b=>b.addEventListener("click",()=>showStep(Math.min(current+1,screens.length-1))));
...
function playBlowSound(){
  try{
    ...
    const fil=ctx.createBiquadFilter();fil.type="bandpass";fil.frequency.setValueAtTime(1000,now);fil.frequency.exponentialRampToValueAtTime(360,now+d);
    const g=ctx.createGain();g.gain.setValueAtTime(.0001,now);g.gain.exponentialRampToValueAtTime(.18,now+.07);g.gain.exponentialRampToValueAtTime(.0001,now+d);
    src.connect(fil);fil.connect(g);g.connect(ctx.destination);src.start(now);src.stop(now+d);
...
let blown=false,micStream=null,micCtx=null,micFrame=null;
...
function extinguish(){
  ...
  tapFallbackBtn.disabled=true;tapFallbackBtn.style.opacity=".4";
  floatBurst(34,["♡","✦","🎀","✨"]);
  confetti();
  buzz([30,20,40]);
...
async function startBreath(){
  ...
  try{
    ...
    // Short baseline calibration to reduce accidental triggers.
    let samples=[],calibrating=true,baseline=.02,strongMs=0,last=performance.now();
    ...
    function loop(now){
      ...
      if(calibrating){
        ...
        strongMs = rms>threshold ? strongMs+dt : Math.max(0,strongMs-dt*1.6);
        if(strongMs>210){extinguish();return}
      }
      last=now;
...
document.getElementById("hugBtn").addEventListener("click",()=>floatBurst(36,["♡","♥","🎀","✦"]));
...
function confetti(){
  ...
  const p=Array.from({length:130},()=>({
    x:Math.random()*c.width,y:-20-Math.random()*c.height*.45,
    vx:-1.1+Math.random()*2.2,vy:2.5+Math.random()*4,
    w:4+Math.random()*5,h:7+Math.random()*8,r:Math.random()*6.28,vr:-.12+Math.random()*.24,
    a:1,h:[330,345,285,45][Math.floor(Math.random()*4)]
  }));
  let f=0;
  function draw(){
    x.clearRect(0,0,c.width,c.height);
    p.forEach(q=>{
      q.x+=q.vx;q.y+=q.vy;q.r+=q.vr;if(f>135)q.a-=.012;
      x.save();x.translate(q.x,q.y);x.rotate(q.r);x.globalAlpha=Math.max(0,q.a);
      x.fillStyle=`hsl(${q.h} 85% 76%)`;x.fillRect(-q.w/2,-q.h/2,q.w,q.h);x.restore();
    });
    f++;if(f<220)requestAnimationFrame(draw);
  } draw();
}
